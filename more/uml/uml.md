...
Презентация к [лекции](https://youtu.be/kFVT8fRwjzg) (смотреть лучше на скорости 1.75).
...
💡 Подробности реализации не влияющие на понимание взаимодействия (например Петю и Мишу мы инстанцируем с помощью абстрактной фабрики) тоже следует исключить.
...
В UML есть и другие диаграммы. Они бывают нужны несколько реже, чем три описанные выше. Используйте то, что вам подходит. Но помните, что читатель может быть плохо знаком с некоторыми обозначениями. Комментируйте важные аспекты, не исключайте текстовое описание полностью.
