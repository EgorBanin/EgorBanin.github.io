...
if ($message !== null) { // если нам прислали сообщение
	mail(
		...
		wordwrap($message, 70, "\r\n", true) // собственно само сообщение (к сообщению есть особые требования, читайте документацию https://php.net/mail)
	);
...
// если сообщение не присылали или есть ошибки, то окажемся тут и отобразим форму
...
Сейчас у формы есть недостаток -- если перезагрузить страницу после отправки, то форма будет отправлена повторно. Чтобы этого избежать, можно сделать редирект (перенаправление). Редирект это ответ сервера, в котором он просит клиента (браузер) выполнить другой запрос. Обычно редиректы используются для перенаправления в случае неправильного запроса страницы (например если набрать https://php.net/redirect, то сайт перенаправит нас на другую страницу https://www.php.net/manual/ru/function.header.php).
...
Страницу thanks.php тоже надо добавить. 
